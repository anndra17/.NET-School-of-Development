...
public sealed class CsvReader : ICsvReader
{
    ...
    public CsvReader(IFileSystem fs, ISystemLogger log)
    {
        _fileSystem = fs ?? throw new ArgumentNullException(nameof(fs));
        _systemLogger = log ?? throw new ArgumentNullException(nameof(log));
    }
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            _systemLogger.Warn($"Missing CSV: {path}");
            yield break;
        }
        ...
        while ((line = await reader.ReadLineAsync()) is not null)
        {
            ...
            {
                _systemLogger.Warn($"[{path}] Line {lineNumber}: {ex.Message}");
                continue;
            }
            ...
            if (!headerProcessed)
            {
                if (LooksLikeHeader(cells))
                {
                    ...
                    {
                        _systemLogger.Warn($"[{path}] Different header at line {lineNumber} from the canonical one. Continues with positions in the order found.");
                    }
                    continue;
                }
                else
                {
                    _systemLogger.Warn($"[{path}] Missing header; The first line of data is {lineNumber}.");
                    headerProcessed = true;
                }
            }
